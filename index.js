...
//   DISCORD_TOKEN, DISCORD_CLIENT_ID, TZ=America/New_York, POLYGON_KEY (optional)
...
async function polygonQuote(ticker) {
  if (!POLY) return null;
  try {
    const nb = await axios
      .get(`https://api.polygon.io/v2/last/nbbo/${ticker}`, { params: { apiKey: POLY } })
      .then((r) => r.data?.results);
    const price = nb ? (nb.bid.price + nb.ask.price) / 2 : null;
    if (!price) return null;
    const prev = await axios
      .get(`https://api.polygon.io/v2/aggs/ticker/${ticker}/prev`, { params: { apiKey: POLY } })
