...
// Low-cap scan posts as a clean embed.
// -----------------------------------------------------------------------------
...
async function yahooQuoteFull(ticker) {
  try {
    ...
    const q = await yahooFinance.quoteSummary(ticker, { modules: ["price", "summaryDetail"] });
    const p = q?.price;
    ...
    const chg =
      p.regularMarketChangePercent ?? p.postMarketChangePercent ?? p.preMarketChangePercent ?? 0;
      ...
      if (POLY) {
        try {
          const info = await axios
            .get(`https://api.polygon.io/v3/reference/tickers/${t}`, { params: { apiKey: POLY }, timeout: 6000 })
            .then((r) => r.data?.results);
...
client.on("interactionCreate", async (i) => {
  ...
  try {
    if (i.commandName === "alert") {
      ...
      // ✅ FIX: null-proof input so it never crashes
      const rawText = i.options.getString("text");
      const text = typeof rawText === "string" && rawText.trim().length ? rawText : "NVDA";
